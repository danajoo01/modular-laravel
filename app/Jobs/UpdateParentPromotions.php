...
class UpdateParentPromotions extends Job implements ShouldQueue
{
...
//                                        ->whereRaw("date(promotions_template.createddate) < '2016-08-03'")
        ...
        foreach($promotions as $promotion) {
            $pselect = PromotionCondition::where('voucher_condition_id', $promotion->promotions_condition_parent_id)
                                        ->first();
        ...
        if(!empty($active_job)) {
            ...
            if($max_voucher_id == 0) {
                \Log::info('there is no exist voucher_condition_update row');
                exit;
            }

            ActiveJob::where('table_name', '=', 'promotions_condition_update')
                        ->update(['last_row' => $max_voucher_id]);
        } else {
            ActiveJob::create([
                    'table_name' => 'promotions_condition_update',
        ...
        if($this->continue == true) {
            ...
            app('App\Modules\Promotion\Controllers\PromotionController')->updatePromotionConditionParent(0);
        }
    }
}
