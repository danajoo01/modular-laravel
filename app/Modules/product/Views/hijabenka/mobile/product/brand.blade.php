    	...
    	<ul>
            ...
            <select id="sizes">
            	...
            	@if(count($size)) > 0)
                    @foreach($size as $row)
                    <?php 
                        $size_url   = isset($row->product_size_url) ? $row->product_size_url : str_replace(' ','_',strtolower($row->product_size));
                        $size_name  = isset($row->product_size) ? $row->product_size : str_replace('_',' ',strtoupper($row->product_size_url));
                    ?>
            		<option value="{{ $size_url }}">{{ $size_name }}</option>
                	@endforeach
                @endif
            </select></li>
        </ul>
...
@section('marketing-tag-body')
    @if(getMarketingEnv() == true)
        @include('marketing-tag.hijabenka.mobile.brand')
