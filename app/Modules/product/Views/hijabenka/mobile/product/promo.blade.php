...
@section('css')
<link rel="stylesheet" href="{{ asset('hijabenka/mobile/css/catalog.css?t=').date('YmdHis') }}">
@endsection
...
<h1><i class="fa fa-search" aria-hidden="true"></i>Filter<i class="fa fa-times" aria-hidden="true"></i></h1>
	<div class="filter-list" id="sticky-list">
    	...
    	<ul>
          <li class="has-sub filter-gender">
        		<a href="">Gender<div class="display"></div></a>
            <ul>
            	<li>
                <input type="radio" name="gender" value="Pria|men" id="RadioGroup-men" onclick="genderAction(this)">
                <label for="RadioGroup-men">Pria</label>
              </li>
              <li>
                <input type="radio" name="gender" value="Wanita|women" id="RadioGroup-women" onclick="genderAction(this)">
                <label for="RadioGroup-women">Wanita</label>
              </li>
              <li>
                <input type="radio" name="gender" value="Semua|all" id="RadioGroup-all" onclick="genderAction(this)">
                <label for="RadioGroup-all">Semua</label>
              </li>
            </ul>
          </li>
            ...
            <?php /*
            <li class="has-sub filter-brand"><a href="">Brand<div class="display"></div></a>
            <ul id="ul-brand">
                ...
                ?>
            	<li>
                	<input type="checkbox" name="brands" value="{{ $row->brand_name }}" id="RadioGroup-{{ $brand_url }}" data-url="{{ $brand_url }}" onclick="brandAction(this)">
                    <label for="RadioGroup-{{ $brand_url }}">{{ $row->brand_name }}</label>
                </li>
                @endforeach
            </ul>
            </li>
	...
	<ul>
        ...
        <li>
            <select id="sort">
                ...
                <?php 
                $default_filter = '';
                if(!isset($_GET["price"]) && !isset($_GET["pn"]) && !isset($_GET["discount"]) && !isset($_GET["popular"]) && !isset($_GET["recommended"])){
...
<div class="catalog-banner-wrapper">
    <div class="catalog-banner">
        @if (!is_null($special) && isset($special->special_page_image_banner) && $special->special_page_image_banner != '')
            <a href="#" {!! (\Request::get('page') != null && \Request::get('page') > 0) ? "style='display:none;'" : "" !!}>
                <img src="{{ IMAGE_SPECIAL_PAGE_UPLOAD_PATH }}banner/{{ $special->special_page_image_banner }}" style="width: 100%;">
            </a>
            @if($special['special_page_desc'] != "" && \Request::get('page') == null &&  \Request::get('page') == 0)
            <span>{!! $special['special_page_desc'] !!}</span>
            @endif
        @endif
    </div>
</div>
            ...
            <li>
                <a href="{{ $url }}">
                    <div class="catalog-img">
                        ...
                        @if (isset($row->set_display_limited_item_set_hb) && $row->set_display_limited_item_set_hb == 1 && (int) $inventory > 0 && $row->product_status <> 2)
                            @if(! stristr($set_display_limited_item_category_hb, $row->type_url) === FALSE && (int) $inventory < $set_display_limited_item_minimal_hb)                                                                                                    
...
@section('marketing-tag-body')
    @if(getMarketingEnv() == true)
        @include('marketing-tag.hijabenka.mobile.promo-body')
        @include('marketing-tag.hijabenka.mobile.category-page', ['catalog' => $catalog])
    @endif
@endsection