		...
		<div class="wrapper">
        	<div class="content-header">
                ...
                <!-- <div class="sortby">
                    <select id="sort-by" data-sort="true" onchange="ChangeUrl(this)">
                        ...
                        <option value="pn=desc">Produk Terbaru</option>
            ...
            <div class="catalog-wrapper">
                ...
                @foreach ($catalog as $row)
                    ...
                    ?>
                    <li id="li-catalog">
                        <a href="{{ $url }}" class="catalog-img" onClick="onProductClick({{ $row->pid }}, '{{ $row->product_name }}', '{{ $row->url_set }}', '{{ $row->brand_name }}', '', {{ $index }}, 'Special Page', '{{ $url }}')">                                                       
                            <?php
                            $imageoverlay = '';
                            ...
                            @if (isset($row->set_display_limited_item_set_sd) && $row->set_display_limited_item_set_sd == 1 && (int) $row->inventory > 0 && $row->product_status <> 2)
                                <?php
                                ...
                                ?>
                                @if(! stristr($set_display_limited_item_category_sd, $row->type_url) === FALSE && (int) $row->inventory < $set_display_limited_item_minimal_sd)
...
<?php
    ...
    $catalog_json = json_encode($catalog_ids);
?>
@if(!empty($user))
...
@if(getMarketingEnv() == true)
    @include('marketing-tag.shopdeca.desktop.promo-page', ['catalog_ids' => $catalog_json])
@endif
...
@section('marketing-tag-body')
    @if(getMarketingEnv() == true)
        @include('marketing-tag.shopdeca.desktop.promo-body')
    @endif
@endsection