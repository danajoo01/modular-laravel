...
@section('meta')
	...
	<meta property="og:site_name" content="shopdeca.com" />	
                            ...
                            <ul>
                                ...
                                @if(isset($fetch_product_size) && !empty($fetch_product_size))
                                    ...
                                    <li @if($rows->inventory <= 0) title="Habis Terjual" @endif>
                                        <div @if($rows->inventory <= 0) style="background-color:#dedede;" @endif><input type="checkbox" name="size_category" value="{{ $rows->product_size }}" id="size-{{ $rows->product_size_url }}" class="size-filter size-{{ $rows->product_size_url }}" @if($rows->inventory <= 0) disabled @endif>
                                          <label for="size-{{ $rows->product_size_url }}" id="{{ $rows->product_sku }}" @if(!$rows->inventory <= 0) onclick="getSKU(this.id);" @endif>{{ $rows->product_size }}
...
@if(getMarketingEnv() == true)
    @include('marketing-tag.shopdeca.desktop.product-page', ['catalog_ids' => json_encode($catalog_json), 'mkt_product_id' =>json_encode(isset($fetch_product->pid) ? $fetch_product->pid : '')])
@endif
