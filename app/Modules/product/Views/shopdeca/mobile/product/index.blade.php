            	...
            	<li>
                	<input type="radio" name="categories" value="{{ $row->type_name_bahasa }}|{{ $row->type_url }}" id="RadioGroup-{{ $row->type_url }}" 
                        @if(!empty(findUriSegment('cat_parent')))
                            {{--*/ $get_key = array_search($row->type_url, findUriSegment('$arr_key_val')) /*--}}
                            @if($row->type_url == findUriSegment('$arr_key_val')[$get_key])
                            checked="checked"
                            @endif
                         @endif
                    >
...
@if(getMarketingEnv() == true)
    @include('marketing-tag.shopdeca.mobile.category-page', ['catalog' => $catalog])
@endif
