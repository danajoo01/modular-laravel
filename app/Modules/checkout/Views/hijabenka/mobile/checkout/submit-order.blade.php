...
<link rel="stylesheet" href="{{ asset('hijabenka/mobile/css/lebaran-notice.css') }}">
<style>
  .sub-btn{
        ...
        <h2>Transfer Reguler (Disetujui Manual)</h2>
          <ul>
            @foreach($list_payment_method_bank_transfer as $bank_transfer)
              ...
              <li>
                <label class="clear">
                  <input class="rad-payment-method" name="rad-payment-method" type="radio" id="RadioGroup2_0" value="{{ $bank_transfer->master_payment_id }}" {{(session('payment_method') == $bank_transfer->master_payment_id) ? "checked" : "" }} name="RadioGroup2">
...
@if(getMarketingEnv() == true)
    @include('marketing-tag.hijabenka.mobile.submit-order')
@endif
