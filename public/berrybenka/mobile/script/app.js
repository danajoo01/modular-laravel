...
function urlGetSegment() {
    var fullPathUrl = window.location.pathname;
    var separators = ['\\\&', '\\\?'];
...
/* Cookie to trigger register mweb error message */
$(document).ready(function()
{
    ...
    if ($.cookie("right") && $.cookie("left"))
    {
        var leftSelector = $.cookie("left");
        var rightSelector = $.cookie("right");

        $(leftSelector).addClass('slide-left');
        $(rightSelector).addClass('slide-right');

        $.cookie('left', '', { expires: -7, path: '/' });
        $.cookie('right', '', { expires: -7, path: '/' });
    }
});
