...
//get enter keypress for berrybenka
// if (window.location.hostname.includes("berrybenka")){
...
$('#moe-push-div').on('click', 'span:nth-child(3)', function(){
    var bounce_exist = document.getElementsByClassName('bouncex_header_top').length;
    var bounce_visible  = $('.bouncex_header_top').is(":visible");
    
    console.log('clicked');
    if(bounce_exist == 0) {
        $('header').css('cssText', 'margin-top: 0 !important');
    } else if(bounce_visible) {
        $('header').css('cssText', 'margin-top: 97px !important');
    }
});
...
function sendVal(val,type,filter,name,productParent,genders){
  ...
  var gender = $('li.ui-state-active > a.menu > span').text().toLowerCase().replace(' ','');
...
function searchSolr(id,form){
  // var gender = $('li.ui-state-active > a.menu > span').text().toLowerCase().replace(' ','');
  var gender = $(".menu-aktif").attr("gender");
  ...
  // $(formid+" #"+id).autocomplete($(formid+" #"+id).attr('url'), {maxdata:50, selectFirst: false, width: "1080px", tops: "100%", lefts: "10px", scrollHeight: "260px", delay:false, divResults:".bottom-header > .wrapper > #tabs > .right-item-wrapper", extraParams:{"gender":gender}});
  $(formid+" #"+id).autocomplete($(formid+" #"+id).attr('url'), {maxdata:50, selectFirst: false, position: "inherit", width: "100%", tops: "null", lefts: "null", scrollHeight: "260px", delay:false, divResults:".search-wrapper", extraParams:{"gender":gender}});
...
function search_bb(keyword){
  var nkey = keyword.replace(" ", "+").replace("&", "-and-");
...
/* Write value to globale variable for impression*/
function writeImpression(data, ref, counter) {
    var index_data = counter - 1;

    catalog_data930ad9[index_data].id       = data.pid;
    catalog_data930ad9[index_data].name     = data.product_name;
    catalog_data930ad9[index_data].category = data.category;
    catalog_data930ad9[index_data].brand    = data.brand_name;
    catalog_data930ad9[index_data].list     = ref;
    catalog_data930ad9[index_data].position = counter;
}

  function ChangeMenu(elm) {

    var gender = elm.getAttribute("gender");

    if(gender == 'wanita'){
      $("#togle-wanita").addClass('menu-aktif'); 
      $("#togle-pria").removeClass('menu-aktif'); 
      $(".menu-woman").show();
      $(".menu-men").hide();
    }else{
      $("#togle-wanita").removeClass('menu-aktif'); 
      $("#togle-pria").addClass('menu-aktif');
      $(".menu-woman").hide();
      $(".menu-men").show();
    }
