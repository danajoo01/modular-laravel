...
function getCheckedVal(elm) {
    ...
    if(checked > 0) {
        ...
        if(elm == 'harga'){
            var checkedUrl = $('#ul-'+ elm +' input:radio:checked').map(function() {
...
function build_price_uri(param){
    var priceuri = '';
    $.each(param, function(index, value) {
        priceuri += value.replace("sprice=", "") + '|'; 
    });

    return 'sprice=' + priceuri.slice(0, -1);
}
...
function ChangeUrl(elm) {
    ...
    if(typeof harga != 'undefined') {
        ...
        // console.log(me.getAttribute("harga-param"));
        ...
        // var checked = $('#ul-harga > li > input:radio:checked').map(function() {
        //     return this.value
        // }).get();
        ...
        uri_gender = "";
        hargaUrl = getCheckedVal('harga');
        sortUri = price_action(hargaUrl, 'sprice');

        if(sortUri == "?"){
            sortUri = "";
        }

        urlPriceFlag.val(hargaUrl);
    ...
    if(typeof sortUri != 'undefined') {
        ...
        if(typeof currentUri.split('?')[1] != 'undefined'){    
            ...
            if(typeof varSort[0] != 'undefined' && typeof varSort[1] != 'undefined' && available_sort.indexOf(varSort[0]) >= 0 && available_method.indexOf(varSort[1]) >= 0){
                ...
                if(urlPriceFlag.val() != ''){
                    sortUri = '?' + urlPriceFlag.val() + '&popular=desc';
                }
            }            
...
function append_catalog(data,wishlist,domain_alias, ref, page_num=0) {
    ...
    $.each(data, function(index, value) {
        ...
        if(domain_alias == 'bb'){
            pid_eksklusif = value.eksklusif_bb;
        }else if(domain_alias == 'hb'){
            pid_eksklusif = value.eksklusif_hb;
        }
